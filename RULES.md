...
11. Only for a legacy plan that explicitly declares legacy `provenance_level: v3-required`, or has an existing legacy audit trail, keep the `boundary-contract/v1` audit order（**2026-07-28 更新**：v1 已迁移至 `audit-boundary-matrix/v3`）separate: frozen contract → case evidence → `audit-boundary-precheck` → `MODEL_REVIEW` → `validate-audit`. A `BLOCKED` matrix or nonzero validator is never overridable by model prose. New work defaults to `.agents/skills/outcome-governance/SKILL.md`: freeze outcome, boundary, and fixed acceptance; amend changes; independently execute tests. This structural path is `review-separated`, not runtime admission, and does not govern implementation mechanics.
