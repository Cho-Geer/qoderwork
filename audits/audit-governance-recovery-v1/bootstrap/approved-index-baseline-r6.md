...
**Status**: `BLOCKED`
**Admission state**: `P1_R6_PENDING_REFREEZE` (was P1_R5_PENDING_REFREEZE; superseded by 2026-07-31 materialization idempotent-put revision)
...
**Canonical contract SHA-256**: `PENDING_R6_FREEZE` (recomputed after 2026-07-31 materialization idempotent-put revision; see §1.5 P0 preflight step 6)
**Approved plan files**: `audits/audit-governance-recovery-v1/approved-plan-files-r6.sha256`
**Approved plan files SHA-256**: `PENDING_R6_FREEZE`
**Approved index baseline**: `audits/audit-governance-recovery-v1/bootstrap/approved-index-baseline-r6.md`
**Approved index baseline SHA-256**: `PENDING_R6_FREEZE`
**Approval request**: `audits/audit-governance-recovery-v1/approval-request-r6.json`
**Approval request SHA-256**: `NOT_EMBEDDED_ACYCLIC_PENDING_R6`
**Approval decision**: `audits/audit-governance-recovery-v1/approval-decision-r6.json`
**Approval decision SHA-256**: `PENDING_R6_FREEZE`
...
Auditor Session A MUST run this preflight before the r6 admission freeze. Each step has a hard-gate check; any failure blocks admission and returns a `STATUS_PUBLICATION_UNAUTHORIZED` verdict (not BLOCKED).
...
2. **9-r6 artifacts present** (the post-revision generation is r6 because canonical L1650 was revised 2026-07-31 for idempotent-put; the original 9-r5 path layout becomes 9-r6):
   - `audits/audit-governance-recovery-v1/approved-plan-files-r6.sha256`
   - `audits/audit-governance-recovery-v1/approved-plan-object-set-r6.json`
   - `audits/audit-governance-recovery-v1/bootstrap/approved-index-baseline-r6.md`
   - `audits/audit-governance-recovery-v1/bootstrap/m1-p0-freeze-manifest-r6.json`
   - `audits/audit-governance-recovery-v1/bootstrap/p4-boundary-r6.json`
   - `audits/audit-governance-recovery-v1/bootstrap/approved-plan-materialization-r6.json`
   - `audits/audit-governance-recovery-v1/approval-request-r6.json`
   - `audits/audit-governance-recovery-v1/approval-decision-pending-r6.json`
   - `audits/audit-governance-recovery-v1/approval-decision-r6.json`
...
6. **r6 plan-files hash recorded**: `sha256sum plans/audit-governance-recovery-v1/canonical-requirements-contract.yaml plans/audit-governance-recovery-v1/formal-plan-set/*.md` 输出到 `audits/audit-governance-recovery-v1/approved-plan-files-r6.sha256`，与 manifest 字段一致。
...
All "*-r5.*" path references are now `*-r6.*` because the 2026-07-31 materialization idempotent-put revision (canonical L1650) changes the canonical SHA-256 and therefore every artifact that references it (object-set, plan-files, pending, request, decision, index-baseline; freeze-manifest generation 5→6, p4-boundary renamed only). The r5 freeze artifacts remain in `audits/audit-governance-recovery-v1/` for historical reference but the r6 freeze is the new authoritative generation.
