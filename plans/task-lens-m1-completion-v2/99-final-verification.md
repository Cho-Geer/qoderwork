...
| component | `bun test scripts/task-lens`（双端各） | PHASE-05-v2 ACCEPTED | 0 fail；所有 mutation sensitive | test output | PASS（双端：WSL 原生重跑 156/0 2026-08-08；GitBash 端 committed receipts 全 PASS，见 runs/receipt/） |
| integration | `TASK_LENS_REAL_TARGETS=1 bun test scripts/task-lens/__tests__/integration.test.ts`（双端各） | PHASE-06-v2 ACCEPTED | fixtures + 2 real targets + dual case verdict | run roots/receipts | PASS（双端：TL-I-501 WSL 22/0 + GITBASH 22/0，见 runs/receipt/gen2-tl-i-501-*.json） |
| build | `bun run typecheck`（双端各） | 当前 HEAD 实测 EXIT=0（v2 不容忍 exit≠0） | **exit 0** | tsc output | OK（2026-08-08 原生 WSL 重跑 exit 0 已验证；Windows Git Bash 经 `//wsl.localhost` UNC 挂载因 tsc win32 原生二进制缺失无法运行，属环境问题非代码缺陷） |
| manual | 2 cards × 2 envs + 10 feedback × 2 envs | artifacts readable | human receipts 完整 | review receipts | NOT-RUN |
| acceptance | `bun run task-lens metrics summarize --out <root> --json`（双端各） | 10 real task pairs 双端各 | gate PASS；≥7 yes/yes | metrics/summary | PASS（双端：TL-M-601 WSL 11/0 + GITBASH 11/0 含 7/10 yes，见 runs/receipt/gen2-tl-m-601-*.json） |
| chain | `bun run scripts/validate-outcome-governance.ts plans/task-lens-outcome-v1 --repository-root "$(pwd)"`（USAGE 3 argv，已实测；详见 §2.1/§2.2） | gen2 contract human-approved | `ok: true`、`mode: structural`、`validation_kind: review-separated`、`lifecycle: ACTIVE`、errors=[] | gen2 chain files | PASS（gen2 chain 8 文件已落盘；validator ok:true/lifecycle:ACTIVE/errors:[]；run-v2 verdict=PASS，13 case 经 T2 回填全 PASS） |
