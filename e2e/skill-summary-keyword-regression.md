...
> ⚠️ **STATUS: HISTORICAL FLAWED CAPTURE SNAPSHOT (deprecated as primary reference)** — 本矩阵 ses_0b16* 数据于 2026-07-11 **捕获缺陷期**取样（bridge 竞态 + `coldStartDbFallback` 死代码），含 F6 污染假阳与 dispatch/investigate 漏捕假阴。2026-07-12 修复后 ses_0ae3* 全量重跑已升格为 **post-fix canonical baseline**（见 `e2e-evidence/L1-001-skill-summary/RESULTS.md`）。本文件仅作修复前后对比，不再作为主参照。
