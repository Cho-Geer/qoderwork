...
|---|---|---|---|
| TSI-01 run context / manifest | ✅ 已实施（component） | component | P0-3-01 审计 ACCEPT（2026-07-22）：`setRunState` 加 TRANSITION_TABLE 守卫，非法转换 throw `illegal run state transition: SOURCE -> TARGET`；`createRunContext` 先推导 run ID 再 `existsSync` 检查重复根目录，重复时 throw `run id already exists: RUN_ID`（端口预留前）；manifest 字段断言、PID 身份校验、sse-daemon FRAMEWORK_DB_PATH 缺失 throw、H2_AUTHORIZED 零赋值均已覆盖。28 component tests PASS。 |
