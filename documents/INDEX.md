...
| **blueprints/blueprint-dynamic-path-resolution.md** | **路径动态化与跨平台配置收敛** | **以 2026-07-25 历史快照、2026-07-26 的 1,805 处/467 文件复核及 CodeGraph 调用链边界、受校验解析契约、IDE 适配 gate 与负例矩阵，指导运行时路径迁移；尚未实施。** | **~251** |
| **plans/path-dynamic-resolution-outcome-v1/outcome-contract.json** | **路径动态化 Outcome v1 合同** | **outcome-governance v1 重建：M1 已交付解析契约冻结为验收基线（4 固定组件测试 T-001..T-004）；详见 blueprints/blueprint-dynamic-path-resolution-outcome-v1.md。** | **~5 工件** |
| **plans/path-dynamic-resolution-m1/00-plan-index.md** | **路径动态化 M1 确定性实施计划** | **M1 实施完成（4 phase 全部 ACCEPTED，validate-phase-progression exit 0）：冻结清单 2504 unique entries + 解析器 workspace-paths.ts + start-serve.ts 接入 + test-serve 4 文件接入 + 后续 PLAN_SET 交接。详见 `audits/path-dynamic-resolution-m1/2026-07-29-phase-01-audit.md` / `phase-02` / `phase-03` / `phase-04-audit.md` 与 `continuation-register.json`（14 m1_paths + 55 deferred 含 4 IDE LOCAL_CONFIG）；2026-08-03 SUPERSEDED_BY_OUTCOME_GOVERNANCE_V1** | **~119（入口）** |
