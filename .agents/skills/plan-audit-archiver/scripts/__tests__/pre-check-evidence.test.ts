...
describe("pre-check-evidence contract input", () => test("accepts one nested listed v3 receipt and blocks a missing receipt", () => {
  ...
  const receipt = { schema_version: "audit-evidence-receipt/v3", document_kind: "evidence-receipt", id: "EV-001", audit_id: "audit-001", generation: 1 }; writeFileSync(receiptPath, JSON.stringify(receipt));
...
test("precheck rejects a ledger payload mutation and duplicate receipt id", () => {
  const root = mkdtempSync(join(tmpdir(), "precheck-")); const file = join(root, "ev.json"), receipt = { schema_version: "audit-evidence-receipt/v3", document_kind: "evidence-receipt", id: "EV-001", audit_id: "audit-002", generation: 1, command: "cd /x && bun test" }; writeFileSync(file, JSON.stringify(receipt));
...
test("orphan receipt is reported as a warning only", () => {
  const root = mkdtempSync(join(tmpdir(), "precheck-")); const contractPath = join(root, "contract.json"); writeFileSync(contractPath, JSON.stringify({ audit_id: "audit-003", generation: 1, requirements: [], evidence_receipts: [] })); writeFileSync(join(root, "orphan.json"), JSON.stringify({ schema_version: "audit-evidence-receipt/v3", document_kind: "evidence-receipt", id: "EV-999" }));
...
test("behavior evidence below its required level is blocked", () => {
  const root = mkdtempSync(join(tmpdir(), "precheck-")); const file = join(root, "ev.json"), receipt = { schema_version: "audit-evidence-receipt/v3", document_kind: "evidence-receipt", id: "EV-001", audit_id: "audit-004", generation: 1, evidence_level: "component" }; writeFileSync(file, JSON.stringify(receipt)); const contractPath = join(root, "contract.json"); writeFileSync(contractPath, JSON.stringify({ audit_id: "audit-004", generation: 1, requirements: [{ kind: "BEHAVIORAL", required_evidence_level: "integration", positive_control: { evidence: "EV-001" } }], evidence_receipts: [{ ...receipt, path: "ev.json", sha256: sha(file) }] })); expect(precheckContract(contractPath).issues).toContain("EVIDENCE_RECEIPT_LEVEL_TOO_LOW"); rmSync(root, { recursive: true, force: true });
});

test("precheck rejects a non-v3 receipt (EVIDENCE_RECEIPT_SCHEMA_INVALID)", () => {
  const root = mkdtempSync(join(tmpdir(), "precheck-")); const file = join(root, "ev.json"), receipt = { schema_version: "1.0", id: "EV-001", audit_id: "audit-005", generation: 1 }; writeFileSync(file, JSON.stringify(receipt)); const contractPath = join(root, "contract.json"); writeFileSync(contractPath, JSON.stringify({ audit_id: "audit-005", generation: 1, requirements: [], evidence_receipts: [{ ...receipt, path: "ev.json", sha256: sha(file) }] }));
  expect(precheckContract(contractPath).issues).toContain("EVIDENCE_RECEIPT_SCHEMA_INVALID"); rmSync(root, { recursive: true, force: true });
});
